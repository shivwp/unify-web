import { Row, Col } from "react-bootstrap";
import { Link } from "react-router-dom";
import Form from "react-bootstrap/Form";
import { useState } from "react";

const JobPosting = ({ setCurrentTab, onInputChange, values }) => {
  const [jobPosting, setJobPosting] = useState("createNewJob");

  const handleJobChange = (e) => {
    setJobPosting(e.target.value);
  };

  return (
    <Col lg={9}>
      <div className="s_nav_body">
        <div className="s_b_head">
          <h2>Getting Started</h2>
          <div className="s_on">Step 1 of 5</div>
        </div>
        <div className="my_skil_head mr_2_sm">
          <h3>What would you like to do?</h3>
        </div>
        <div className="s_b_r">
          <Form.Check
            type="radio"
            id="r_1"
            name="g"
            value="createNewJob"
            checked={jobPosting === "createNewJob" ? true : false}
            onChange={(e) => handleJobChange(e)}
          />
          <Form.Label htmlFor="r_1">
            <p>Create a new job post.</p>
          </Form.Label>
        </div>
        {jobPosting === "createNewJob" && (
          <Row className="justify-content-evenly">
            <Col lg={5} md={6} sm={6}>
              <Form.Label
                htmlFor="Shorterm"
                className="select_card"
                style={{ position: "relative" }}
              >
                <div className="sub_radio" style={{ top: 20, right: 20 }}>
                  <Form.Check
                    type="radio"
                    name="job_type"
                    id="Shorterm"
                    value="short_term"
                    checked={values?.job_type === "short_term" ? true : false}
                    onChange={(e) => onInputChange(e)}
                  />
                </div>
                <div className="sel_icon">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
                    height="16"
                    fill="currentColor"
                    className="bi bi-tools"
                    viewBox="0 0 16 16"
                  >
                    <path d="M1 0 0 1l2.2 3.081a1 1 0 0 0 .815.419h.07a1 1 0 0 1 .708.293l2.675 2.675-2.617 2.654A3.003 3.003 0 0 0 0 13a3 3 0 1 0 5.878-.851l2.654-2.617.968.968-.305.914a1 1 0 0 0 .242 1.023l3.27 3.27a.997.997 0 0 0 1.414 0l1.586-1.586a.997.997 0 0 0 0-1.414l-3.27-3.27a1 1 0 0 0-1.023-.242L10.5 9.5l-.96-.96 2.68-2.643A3.005 3.005 0 0 0 16 3c0-.269-.035-.53-.102-.777l-2.14 2.141L12 4l-.364-1.757L13.777.102a3 3 0 0 0-3.675 3.68L7.462 6.46 4.793 3.793a1 1 0 0 1-.293-.707v-.071a1 1 0 0 0-.419-.814L1 0Zm9.646 10.646a.5.5 0 0 1 .708 0l2.914 2.915a.5.5 0 0 1-.707.707l-2.915-2.914a.5.5 0 0 1 0-.708ZM3 11l.471.242.529.026.287.445.445.287.026.529L5 13l-.242.471-.026.529-.445.287-.287.445-.529.026L3 15l-.471-.242L2 14.732l-.287-.445L1.268 14l-.026-.529L1 13l.242-.471.026-.529.445-.287.287-.445.529-.026L3 11Z" />
                  </svg>
                </div>
                <div className="sho_tit">
                  Short -term or
                  <br /> part-time work
                </div>
                <div className="sm_seds">Less than 30 hrs/week</div>
                <div className="sm_seds">Less than 3 months</div>
              </Form.Label>
            </Col>
            <Col lg={5} md={6} sm={6}>
              <Form.Label
                htmlFor="longterm"
                className="select_card"
                style={{ position: "relative" }}
              >
                <div className="sub_radio " style={{ top: 20, right: 20 }}>
                  <Form.Check
                    type="radio"
                    name="job_type"
                    id="longterm"
                    value="long_term"
                    checked={values?.job_type === "long_term" ? true : false}
                    onChange={(e) => onInputChange(e)}
                  />
                </div>
                <div className="sel_icon">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
                    height="16"
                    fill="currentColor"
                    className="bi bi-calendar-event"
                    viewBox="0 0 16 16"
                  >
                    <path d="M11 6.5a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-1a.5.5 0 0 1-.5-.5v-1z" />
                    <path d="M3.5 0a.5.5 0 0 1 .5.5V1h8V.5a.5.5 0 0 1 1 0V1h1a2 2 0 0 1 2 2v11a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V3a2 2 0 0 1 2-2h1V.5a.5.5 0 0 1 .5-.5zM1 4v10a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1V4H1z" />
                  </svg>
                </div>
                <div className="sho_tit">
                  Designated
                  <br /> longer term work
                </div>
                <div className="sm_seds">More than 30 hrs/week</div>
                <div className="sm_seds">3+ months</div>
              </Form.Label>
            </Col>
          </Row>
        )}

        <div className="s_b_r">
          <Form.Check
            type="radio"
            id="r_2"
            name="g"
            value="editAnExistingDraft"
            onChange={(e) => handleJobChange(e)}
          />
          <Form.Label htmlFor="r_2">
            <p>Edit an existing draft </p>
          </Form.Label>
        </div>
        <div className="s_b_r">
          <Form.Check
            type="radio"
            id="r_3"
            name="g"
            value="reusePreviousJobPost"
            onChange={(e) => handleJobChange(e)}
          />
          <Form.Label htmlFor="r_3">
            <p>Reuse a previous job post</p>
          </Form.Label>
        </div>
        <div className="s_select">
          <select className="carrt_css_select">
            <option>HubSpot Automations - Work flow</option>
          </select>
        </div>
        <div className="btn_foot_sec flex-wrap d-flex">
          <div className="fo_btn_c next_b_btn_c">
            <Link to="/dashboard">
              <button className="mrright-gppnew">Cancel</button>
            </Link>
          </div>
          {!values?.job_type ? (
            <div className="fo_btn_c next_b_btn_c">
              <button disabled className="active_btn_blueDiabled">
                Continue
              </button>
            </div>
          ) : (
            <div className="fo_btn_c next_b_btn_c">
              <button
                className="active_btn_blue"
                onClick={() => setCurrentTab("title")}
              >
                Continue
              </button>
            </div>
          )}
        </div>
      </div>
    </Col>
  );
};
export default JobPosting;
