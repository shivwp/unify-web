...
import { getTransectionHistory } from "../../../../redux/actions/freelancerAction";
...
const Screen = () => {
  ...
  const transectionHistory = useSelector(
    (state) => state.freelancer.transectionHistory
  );

  useEffect(() => {
    const data = {};
    dispatch(getTransectionHistory(data));
  }, []);

  const calculatedAmount = () => {
    let total = 0;

    transectionHistory?.data?.forEach((e) => {
    ...
    return (
      <>
        <Table className="trx_tbl_freelancer mt-3">
          ...
          <tbody>
            {transectionHistory?.data?.map((item, key) => (
              <tr>
        ...
        >
          <div className="transaction_head_n">
            ...
            <div className="trans_h_b_n">
              Balance: <span>${calculatedAmount().toFixed(2)}</span>
            </div>
          </div>
          ...
          <div className="d-flex justify-content-between align-items-center flex-wrap mb-0 mt-0 pb-0">
            ...
            <div className="download_lnk_csx">
              <div className="btn_foot_sec p-0 justify-content-center flex-wrap no-border mt-2">
                ...
                <div className="fo_btn_c next_b_btn_c">
                  <button className="active_btn_blue">Download Invoices</button>
                </div>
              </div>
            </div>
          </div>
